...
<table class="table table-striped table-bordered " style="text-align:center;"  style="width:100%" id="tabla_mensualidades">
    ...
    <tbody>
        ...
        <tr>
          <td hidden>{{$item->id_mensualidad}}</td>
          <td>{{$item->tipo_mensualidad}}</td>
          <td>{{$item->valor}}</td>
          <td>
            <form method="post" action="{{url('/mensualidades/'.$item->id_mensualidad)}}">
                {{csrf_field() }}
           
                 <a href="{{ url('/mensualidades/'.$item->id_mensualidad).'/edit'}}" class="btn bg-primary"><i class="fas fa-edit"></i></a>
...
document.getElementById("enlace").onclick = function () {
    window.location.href = "{{url('mensualidades/create')}}";
};
